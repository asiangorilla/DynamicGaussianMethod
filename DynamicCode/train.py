...
from DynamicCode.helpers import setup_camera, l1_loss_v1, l1_loss_v2, weighted_l2_loss_v1, weighted_l2_loss_v2, quat_mult, \
    o3d_knn, params2rendervar, params2cpu, save_params
from DynamicCode.external import calc_ssim, calc_psnr, build_rotation, densify, update_params_and_optimizer
...
def get_dataset(t, md, seq):
    ...
    for c in range(len(md['fn'][t])):
        ...
        seg = np.array(copy.deepcopy(Image.open(f"dynamic_data/{seq}/seg/{fn.replace('.jpg', '.png')}"))).astype(np.float32)
...
def initialize_params(seq, md):
    init_pt_cld = np.load(f"dynamic_data/{seq}/init_pt_cld.npz")["data"]
    seg = init_pt_cld[:, 6]
    max_cams = 50
...
def train(seq, exp):
    if os.path.exists(f"./output/{exp}/{seq}"):
        ...
        return
    md = json.load(open(f"./dynamic_data/{seq}/train_meta.json", 'r'))  # metadata
